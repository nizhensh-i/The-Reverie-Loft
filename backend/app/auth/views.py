...
def before_request():
    if current_user:
        _auth_service().touch_user_last_seen(user_id=current_user.id)


@auth.route("/login", methods=["post"])
def login():
    j = request.get_json() or {}
    result = _auth_service().create_login_session(
...
def register(validated_data):
    result = _auth_service().create_user_account(
...
def apply_code():
    ...
    result = _auth_service().create_email_code(email=email, current_user=current_user)
    if not result.ok:
...
def confirm(validated_data):
    result = _auth_service().update_email_confirmation(
        user=current_user,
...
def change_email(validated_data):
    result = _auth_service().update_user_email(
        user=current_user,
...
def change_password(validated_data):
    result = _auth_service().update_user_password(
        user=current_user,
...
def reset_password(validated_data):
    result = _auth_service().update_password_by_email(
        email=validated_data.email,
        code=validated_data.code,
        new_password=validated_data.new_password,
    )
...
def change_password_admin():
    ...
    result = _auth_service().update_password_by_admin(
...
def set_password():
    ...
    result = _auth_service().update_password_for_social_user(
        user=current_user,
